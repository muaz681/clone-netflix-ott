...
class MemberController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth:member', ['except' => ['loginApple', 'handleAppleCallback','store', 'register', 'login', 'showlogin', 'redirectToGoogle', 'handleGoogleCallback', 'redirectToFacebook', 'handleFacebookCallback', '_registerOrLoginUser', 'ForgoteStatus', 'changePassword', 'getPasswordForgote', 'forgate_password', 'getPasswordReset', 'resetPassword', 'ResetStatus']]);
    }
    ...
    public function forgate_password(Request $request)
    {
        ...
        $data = [
            ...
            'email'     => $request->email,
            'content'   => url(app()->getLocale().'/password/reset/?token=' . $token . '&&email=' . $request->email)
        ];
        ...
        try {
            ...
            return redirect()->route('forgot-password-status',[app()->getLocale()]);
        } catch (\Exception $e) {
    ...
    private function getLastWatch($member){
        return cache()->remember('my-watch-history-all-' . $member->id, 60 * 5, function () use ($member) {
            return Media::select('media.*')
                ->join('play_list_logs as rt', 'rt.video_id', '=', 'media.id')
    ...
    private function getFavorites($member){
        return cache()->remember('my-favorites-all-' . $member->id, 60 * 5, function () use ($member) {
            return Media::select('media.*')
            ->join('media_favorites as rt', 'rt.media_id', '=', 'media.id')
            ->where(['rt.member_id' => $member->id])
            ->groupBy('rt.media_id')   
            ->with('featured','featuredL')
            ->get();
    ...
    private function getWishlisted($member){

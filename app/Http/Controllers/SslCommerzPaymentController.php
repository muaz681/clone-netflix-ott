...
class SslCommerzPaymentController extends Controller
{
    ...
    private function update($tran_id, $status)
    {
        ...
        if($status == 'Complete'){
            ...
            $order_for_mail = Order::where('transaction_id', $tran_id)->firstOrFail(); 
            $data = [
                'name'      => auth('member')->user()->name,
                'subject'   => 'Cinebaz Limited: Invoice for Media Order',
                'email'     => auth('member')->user()->email,
                'order'     => $order_for_mail
            ];

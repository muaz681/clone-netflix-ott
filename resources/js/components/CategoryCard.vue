<template>
  ...
  >
  <a :href="$site_url + local + '/details/' + cdata.slug">
    <v-img
      :size="185"
      :class="{ 'rounded-b-none shadow': hover }"
      alt="Image"
      :src="getFile(cdata.featured_l.small)"
      v-if="cdata.featured_l && cdata.featured_l.small"
    >
    </v-img>
    <v-img
    ...
    >
      <div class="flex items-center justify-between hover-part-image">
        <div class="flex items-center">
          <v-btn
            v-if="cdata.video_url != null"
            class="bg-white text-black cinebaz-det-fvrt"
            outlined
            fab
            :href="$site_url + local + '/details/' + cdata.slug"
          >
            <svg
		...
		C444.801,187.101,434.001,213.101,414.401,232.701z"
          ...
          <v-btn
            v-if="cdata.premium == 1 && cdata.published_at != null"
            class="items-center text-white justify-center cinebaz-det-fvrt"
            outlined
            fab
            color="white"
            @click="AddToCart(cdata.id)"
          >
            <svg
              aria-hidden="true"
              focusable="false"
        ...
        >
          <span class="d-block details-price-page">
            <span class="badge badge-secondary badge-age-limit"
              >{{ paycurrency }} {{ cdata.discount_price }}</span
            >
          </span>
        </v-btn>
      </div>

    ...
    <li class="slide-item">
        <div class="block-images position-relative">
            <div class="img-box slider-img-box">
                        ...
                        <div class="cardPremium">
                            <div class="cpPrice">
                                ...
                                <div class="planbox__header__value">
                                    <span class="price">
                                        {{ $sdata->medias->discount_price ? PayCurrency($sdata->medias->discount_price) : null }}
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                @endif
                <a href="{{ route('frontend.details', [app()->getLocale(), $sdata->medias->slug]) }}">
                    @if ($sdata->medias->featured)
                        <img data-original="{{ asset('storage/' . $sdata->medias->featured->small) }}"
                            src="{{ asset('storage/' . $sdata->medias->featured->small) }}" class="img-fluid"
                            alt="">
                    @else
                    ...
                    @if (auth('member')->check())
                        ...
                        @if ($deadline < $today)
                            <a href="javascript:void(0)"
                                onclick="AddToCart({{ $sdata->medias ? $sdata->medias->id : 0 }})">
                                <span class="btn btn-warning">
